...
class CustomClusterManager<T extends ClusterItem> {
  CustomClusterManager(this._items, this.updateMarkers,
      {Future<Marker> Function(CustomCluster<T>)? markerBuilder,
  ...
  final Future<Marker> Function(CustomCluster<T>) markerBuilder;
  ...
  void _updateClusters() async {
    List<CustomCluster<T>> mapMarkers = await getMarkers();
  ...
  Future<List<CustomCluster<T>>> getMarkers() async {
    ...
    if (stopClusteringZoom != null && _zoom >= stopClusteringZoom!)
      return visibleItems.map((i) => CustomCluster<T>.fromItems([i])).toList();

    List<CustomCluster<T>> markers;
  ...
  List<CustomCluster<T>> _computeClustersWithMaxDist(
  ...
  List<CustomCluster<T>> _computeClusters(
      List<T> inputItems, List<CustomCluster<T>> markerItems,
      {int level = 5}) {
    ...
    markerItems.add(CustomCluster<T>.fromItems(items));
  ...
  static Future<Marker> Function(CustomCluster) get _basicMarkerBuilder =>
      (cluster) async {
        return Marker(
